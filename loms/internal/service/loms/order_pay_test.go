...
func TestPayOrder(t *testing.T) {
	...
	}{
		{
			...
			setup: func(fields *fields, args *args) {
				...
				fields.orderProviderMock.
					...
					SetStatusMock.Expect(ctx, args.orderID, model.StatusPayed).Return(nil)
				fields.stockProviderMock.ReserveRemoveMock.Expect(ctx, order.Items).Return(nil)
			},
			args: args{
				orderID: uuid.New(),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := lomsService.PayOrder(ctx, tt.args.orderID)
